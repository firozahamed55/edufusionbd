 ...
 */
export function useTeachers(page: number, search: string, departmentId: string, sort: Sort) {
  const yearId = useCurrentYearId();
  return useQuery({
    queryKey: queryKeys.teachers.list({ page, search, departmentId, sort: sort ? `${sort.key}:${sort.dir}` : "" }),
    queryFn: () => fetchTeachers(createClient(), { page, search, departmentId, yearId, sort }),
