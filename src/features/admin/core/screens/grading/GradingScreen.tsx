...
export function GradingScreen() {
  ...
  return (
    <div className="flex flex-col gap-5 pb-6">
      <header>
        <div className="flex items-center gap-1.5 text-[13px] text-text-muted"><span>{t("কোর সেটিংস", "Core Settings")}</span><span>›</span><span className="text-text-secondary">{t("গ্রেডিং", "Grading")}</span></div>
        <h1 className="mt-1.5 text-[22px] font-bold text-text-primary">{t("গ্রেডিং স্কিম", "Grading Schemes")}</h1>
        <p className="mt-1 text-[13px] text-text-muted">{t("গ্রেড, GPA ও নম্বর পরিসর নির্ধারণ করুন", "Define grades, GPA and mark ranges")}</p>
      </header>
      ...
      <FormCard title={id ? t("স্কিম সম্পাদনা", "Edit scheme") : t("নতুন গ্রেডিং স্কিম", "New grading scheme")}>
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <label className="flex items-center gap-2 self-end pb-2.5 text-[13px] text-text-secondary"><input type="checkbox" checked={isDefault} onChange={(e) => setIsDefault(e.target.checked)} className="size-4 accent-primary" /> {t("ডিফল্ট স্কিম", "Default scheme")}</label>
          </div>
          <div className="overflow-x-auto rounded-xl border border-border-default">
            <div className="min-w-140">
              <div className="flex items-center gap-2 border-b border-border-default bg-sunken px-3 py-2 text-[12px] font-semibold text-text-muted">
          ...
          <button onClick={() => setScales((p) => [...p, { grade_letter: "", gpa_point: 0, min_marks: 0, max_marks: 0 }])} className="flex items-center gap-1.5 self-start text-[13px] font-semibold text-primary"><Plus size={14} /> {t("গ্রেড যোগ করুন", "Add grade")}</button>
      ...
      <div className="overflow-hidden rounded-2xl bg-surface shadow-e3">
        ...
        {rows.length === 0 ? (
          ...
          <div key={r.id} className={cn("flex items-center gap-3 px-5 py-3.5 border-b border-border-default last:border-0", i % 2 === 1 && "bg-sunken")}>
            <div className="flex-1 text-sm font-medium text-text-primary">{r.name}{r.is_default ? <span className="ml-2 rounded-full bg-success-bg px-2 py-0.5 text-[11px] font-semibold text-success-fg">{t("ডিফল্ট", "Default")}</span> : null}</div>
            <span className="text-[13px] text-text-muted tnum">{n(r.scales.length)} {t("গ্রেড", "grades")}</span>
