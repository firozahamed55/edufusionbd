...
export async function middleware(request: NextRequest) {
  ...
  // Route handlers are fetched by client JS, never navigated to. A 307
  // redirect to an HTML /login page on a missing session silently breaks the
  // caller — `fetch()` follows it and gets HTML where it expected JSON. Every
  // route under `/api` does its own `getUser()` check (see
  // `src/server/sms/sendCampaign.ts` and `src/server/users/accountOps.ts`) and
  // returns a real 401, so the redirect below must not apply here.
  //
  // Was `/api/v1/` when the SMS route was the only handler. `/api/admin/users/*`
  // (Settings audit M-15) lives outside that prefix — it is not a versioned
  // public API, it is this product's own admin surface — and inherited the
  // redirect the moment it was added. Widening to `/api/` is what the comment
  // above always meant.
